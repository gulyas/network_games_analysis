...
def thread_do_all(Q, path, page, link):
    ...
    links = list({'https://en.wikipedia.org{}'.format(a['href']) for a in soup.select('p a[href]') if
...
def find_shortest_path_threaded(start, end):
    path = Manager().dict()
    path[start] = [start]
    Q = deque([start])
...
if __name__ == '__main__':
    ...
    if check_page_threaded(start) and check_page_threaded(end):
